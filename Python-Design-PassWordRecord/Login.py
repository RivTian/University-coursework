...
try:
    ...
    from PyQt5.QtWidgets import QPushButton, QLabel, QLineEdit, QFrame, QMessageBox, QComboBox, QApplication, QWidget
...
class MyWindow(QWidget):
    ...
    def add_button(self):
        ...
        self.login_button.setToolTip(
            'If you are the admin, please login in with the specific account')
    ...
    def set_background_image(self):
        ...
        self.frame.setStyleSheet(
            'background-image: url("images/python.png"); background-repeat: no-repeat; text-align:center;'
        )
    ...
    def login(self):
        ...
        if username and password:  # 如果两个输入框都不为空
            if data:
                ...
                QMessageBox.information(self, 'Error', 'No such username',
                                        QMessageBox.Yes | QMessageBox.No)
        elif username:  # 如果用户名写了
            QMessageBox.information(self, 'Error', 'Input your password',
                                    QMessageBox.Yes | QMessageBox.No)
        else:
            QMessageBox.information(self, 'Error', 'Fill in the blank',
                                    QMessageBox.Yes | QMessageBox.No)

    def sign_up_window(self):
        self.sign_up_win.setWindowIcon(self.icon)
        self.sign_up_win.move(self.x() + 100,
                              self.y() + 100)  # 移动一下注册窗口，以免和之前的重复
        frame = QFrame(self.sign_up_win)
        self.sign_up_win.setWindowFlag(Qt.Dialog)
        frame.resize(1000, 300)
        frame.setStyleSheet(
            'background-image: url("images/python.png"); background-repeat: no-repeat;'
        )
        frame.move(40, 150)
        # 打开注册窗口时，清除原来的信息
...
if __name__ == '__main__':
    ...
    sys.exit(app.exec_())
    Auto_Mail.mail()  # 最后实现自动发送data文件至邮箱进行备份
